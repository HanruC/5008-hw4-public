...
int dll_insert(dll_t *l, int pos, int item)
{
    ...
    new->previous = pointer; // new->prev will be the head which is pointer. 
    new->next = pointer->next; // new->next will be the previous head->next. 
    pointer->next->previous = new; //previous head->next->prev will be the new. 
    pointer->next = new; // previous head next now be the new. 
    l->count++;
    return 1;
}
